...
var StockInfoListViewModel = function () {
    ...
    self.load = function () {
        ...
        AJAXPost(StorefrontUri("api/sitecore/catalog/GetCurrentProductStockInfo"), JSON.stringify(data), function (data, success, sender) {
            ...
            $(document).trigger("ProductCatalogAjaxComplete", ["PC-GetCurrentProductStockInfo-Completed"]);
...
$(function () {
    signForNotificationVM = {
        ...
        load: function () {
            ...
            AJAXPost(StorefrontUri("api/sitecore/account/getcurrentuser"), null, function (data, success, sender) {
                ...
                $(document).trigger("ProductCatalogAjaxComplete", ["PC-GetCurrentUser-Completed"]);
            });
        },
    ...
    var isGetCurrentProductStockInfoCompleted = false;
    var isGetCurrentUserCompleted = false;

    $(document).on('ProductCatalogAjaxComplete', function (e, ajaxCallName) {
        switch(ajaxCallName) {
            case 'PC-GetCurrentProductStockInfo-Completed':
                isGetCurrentProductStockInfoCompleted = true;
                break;
            case 'PC-GetCurrentUser-Completed':
                isGetCurrentUserCompleted = true;
                break;
        }

        if (isGetCurrentProductStockInfoCompleted && isGetCurrentUserCompleted) {
            $(document).trigger("ProductCatalogInitialized");
        }
    });    
